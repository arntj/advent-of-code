...
pub fn solve(races: Races) [2]u64 {
    ...
    for (0..races.times.len) |i| result[0] *= get_ways_to_win(races.times[i], races.distances[i]);

    result[1] = get_ways_to_win(races.times_part2, races.distances_part2);

    return result;
}

test "ways to win when race lasts 7 time units and record distance is 9 distance units" {
    const result = get_ways_to_win(7, 9);
...
test "ways to win when race lasts 71530 time units and record distance is 940200 distance units" {
    const result = get_ways_to_win(71530, 940200);

    try testing.expectEqual(@as(u64, 71503), result);
}

fn get_ways_to_win(time: u64, record_distance: u64) u64 {
    var result: u64 = 0;
